 ...
 * 
 */
package com.nse.temporal.workflow;
 ...
 * 11-May-2024
 * 12:05:09 pm
 * Objective: Its objective is to implement the defined OrderFulfillmentWorkflow.
 * This OrderFulfillmentWorkflowImpl class will override a single method declared within OrderFulfillmentWorkflow interface. Before 
 * overriding this method, we need to prepare followings.
 ...
 *  
 *  <br>Remark: 
 *  <br>[1]All activities participating in the workflow will be registered at Temporal server as an stub and whatever activity details have been set
 *   in its activity option will be executed accordingly by Temporal Server.
 *  <br>[2] Every activity stub creation is using an interface of an activity which will be implemented by the respective activity tackling micro service.
 *   This stub concept is used for making a RPC call from Temporal Server.
 *  <br>[3] Under its workflow method createOrder(), we will call the respective method of different activity, which is actually defined in its respective
 *    micro-service, via this activity stub.
 *  <br>[4] Calling of these stubs method should be in the same sequence in which the business demands.
 *  <br>[5] The workflow initiator is called local activity.
 */
